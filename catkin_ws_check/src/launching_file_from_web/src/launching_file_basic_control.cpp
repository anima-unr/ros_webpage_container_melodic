...
//         ros::init(argc,argv,"roslaunch_launcher_basic_control");
//         ros::NodeHandle n;
       
// 	ros::Subscriber sub_2 = n.subscribe("/launch_basic_control", 1, callback_4);
...
void callback_run(std_msgs::String msg){
    if(!running){
        ...
        Poco::ProcessHandle ph_running = Poco::Process::launch("rosrun", args,0,0,0); //launch a new node
...
int main(int argc, char** argv){
    ros::init(argc, argv, "roslaunch_launcher_basic_control");
    ros::NodeHandle n;
    ros::Subscriber sub_run = n.subscribe("/launch_basic_control",100,callback_run);
    ros::Subscriber sub_kill = n.subscribe("/kill_basic_control",100,callback_kill);
    ros::spin();
    return 0;
}
